...
- **交叉引用**：正文里指向别的节的数字会自动变成链接。会链接的写法：
  ...
  不会链接的写法：紧跟在「编辑器」「版本」后面的数字、后面是「版」的数字、后面是空格或标点的裸数字、行内代码和标题里的数字。`book/book.yml` 的 `xref.versionNumbers` 列出了书里当作版本号用的数字（1.0、1.1、2.0、2.1、2.2），它们只在写成「N.M节」「N.M的」「N.M练一练」时才链接，「（2.1）」「2.1成功」这类模糊写法一律不动；将来编辑器出了 2.3 版，把 `'2.3'` 加进这个列表即可。
